...
class Uart:
    ...
    def wait_for_str_ordered(
        self, msgs: Union[str, list], error_msg: str = "", timeout: int = DEFAULT_WAIT_FOR_STR_TIMEOUT
    ) -> None:
        start = time.time()
    ...
    def wait_for_str(self, msgs: Union[str, list], error_msg: str = "", timeout: int = DEFAULT_WAIT_FOR_STR_TIMEOUT) -> None:
        ...
        while True:
            ...
            if missing_msgs:
                break
            if start + timeout < time.time():
                raise UartLogTimeout(f"{missing_msgs} missing in UART log. {error_msg}\nUart log:\n{self.log}")
            if self._evt.is_set():
                raise RuntimeError(f"Uart thread stopped, log:\n{self.log}")
